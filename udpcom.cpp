...
UDPCOM::UDPCOM(MainWindow *mainw, QString MyAddress1, QString DestineAddress1, int MyPort1, int DestinePort1)
{
    ...
    if(udpSocketIn->isValid()) qDebug() << "UDP Socket IN Opened: " << MyAddress;
    else qDebug() << "Error on UDP Socket IN";

    if(udpSocketOut->isValid()) qDebug() << "UDP Socket OUT Opened "<< DestineAddress;
    else qDebug() << "Error on UDP Socket OUT";
}
